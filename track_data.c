...
int pointertoint (track_node * p){
	int i = ((unsigned int)p - (unsigned int)trackData)/sizeof(track_node);
	if ( i >= TRACK_MAX){
		bwprintf(COM2, "error\n");
	}
	return i;
...
void it_distance(int A, int B){
	if ( (A == B) || (trackData[A].type == NODE_ENTER) || (trackData[A].type == NODE_NONE) || (A >= TRACK_MAX) || (trackData[B].type == NODE_EXIT) || (trackData[B].type == NODE_NONE) || (B >= TRACK_MAX) || (distances[A][B] == MAX_INT)) return;
	if (trackData[B].type == NODE_BRANCH){
		distances[A][pointertoint(trackData[B].edge[DIR_STRAIGHT].dest)] = min(distances[A][B] + trackData[B].edge[DIR_STRAIGHT].dist,distances[A][pointertoint(trackData[B].edge[DIR_STRAIGHT].dest)]);
		distances[A][pointertoint(trackData[B].edge[DIR_CURVED].dest)] = min(distances[A][B] + trackData[B].edge[DIR_CURVED].dist,distances[A][pointertoint(trackData[B].edge[DIR_CURVED].dest)]);
	} else {
		distances[A][pointertoint(trackData[B].edge[DIR_AHEAD].dest)] = min(distances[A][B] + trackData[B].edge[DIR_AHEAD].dist,distances[A][pointertoint(trackData[B].edge[DIR_AHEAD].dest)]);
	}
	return;
}

void init_distance(int distances[][TRACK_MAX],track_node *track){
	int i,j,n,m;
	for (i = 0; i < TRACK_MAX; i++){
		for (j = 0; j < TRACK_MAX; j++){
			if (i != j) distances[i][j] = MAX_INT;
		}
	}
	for (i = 0; i < TRACK_MAX; i++){
		if (track[i].type == NODE_BRANCH){
				distances[i][pointertoint(track[i].edge[DIR_STRAIGHT].dest)] = track[i].edge[DIR_STRAIGHT].dist;
				distances[i][pointertoint(track[i].edge[DIR_CURVED].dest)] = track[i].edge[DIR_CURVED].dist;
		} else if ((track[i].type != NODE_EXIT) && (track[i].type != NODE_NONE)) {
				distances[i][pointertoint(track[i].edge[DIR_AHEAD].dest)] = track[i].edge[DIR_AHEAD].dist;
		}
	}
	
	for (n = 0; n < NUM_ITERATION; n++){
		for (i = 0; i < TRACK_MAX; i++){
			for (j = 0; j < TRACK_MAX; j++){
				if (i != j) it_distance(i,j);
			}
		}
	}
	n = 0;
	...
	for (i = 0; i < TRACK_MAX; i++){
		for (j = 0; j < TRACK_MAX; j++){
			if ( (i == j) || (trackData[i].type == NODE_ENTER) || (trackData[i].type == NODE_NONE) || (i >= TRACK_MAX) || (trackData[j].type == NODE_EXIT) || (trackData[j].type == NODE_NONE) || (j >= TRACK_MAX) ) {
				m++;
				if (distances[i][j] == MAX_INT) n++;
			}
		}
	}
	bwprintf (COM2, "%d out of %d have no paths\n", n,m);
}
